...
const CONFIG_KEY = "asset-tracker-config";
...
async function saveData(records) {
  try { await window.storage.set(STORAGE_KEY, JSON.stringify(records)); } catch (e) { console.error(e); }
}
async function loadConfig() {
  try { const r = await window.storage.get(CONFIG_KEY); return r ? JSON.parse(r.value) : null; } catch { return null; }
}
async function saveConfig(cfg) {
  try { await window.storage.set(CONFIG_KEY, JSON.stringify(cfg)); } catch (e) { console.error(e); }
}
...
export default function AssetTracker() {
  ...
  useEffect(() => {
    Promise.all([loadData(), loadConfig()]).then(([d, cfg]) => {
      setRecords(d);
      if (cfg?.subAccounts) {
        setSubAccounts(cfg.subAccounts);
      } else {
        const defaults = {};
        ASSET_CATEGORIES.forEach(c => { defaults[c.key] = [...c.defaults]; });
        setSubAccounts(defaults);
      }
      setLoading(false);
    });
  }, []);
  ...
  useEffect(() => {
    if (!loading) {
      saveData(records);
      saveConfig({ subAccounts });
    }
  }, [records, subAccounts, loading]);
  ...
  if (loading) return (
    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100vh", fontFamily: "'Noto Sans SC', sans-serif" }}>
